                            ...
                            if pmrp:
                                try:
                                    ...
                                    with sqlite3.connect("./Database/store.db") as db:
                                        cur = db.cursor()
                                    insert = (
                                        "INSERT INTO raw_inventory(product_name, product_cat, stock, mrp, cost_price) VALUES(?,?,?,?,?)"
                                    )
                            ...
                            if pmrp:
                                try:
                                    ...
                                    with sqlite3.connect("./Database/store.db") as db:
                                        cur = db.cursor()
                                    update = (
                                        "UPDATE raw_inventory SET product_name = ?, product_cat = ?, stock = ?, mrp = ?, cost_price = ? WHERE product_id = ?"
                                    )
        ...
        if ename.strip():
            if valid_phone(econtact):
                    if edes:
                        ...
                        messagebox.showerror("OOOPS!", "Please enter designation.", parent=e_add)
            else:
                ...
                if edes:
                    if eadd:
                        if epass:
                            emp_id = vall[0]
                            update = (
                                "UPDATE employee SET name = ?, contact_num = ?, password = ?, address = ?, designation = ? WHERE emp_id = ?"
                                )
                    ...
                    messagebox.showerror("OOOPS!", "Please enter designation.", parent=e_update)
            else:
